...
const ProductDetailPage = () => {
  ...
  const defaultMassas = [
    ...
    { name: 'Amendoim', price: 22 }, { name: 'Red Velvet', price: 32 },
    { name: 'Nozes', price: 38 }, { name: 'Black', price: 42 },
  ];

  const defaultRecheios = [
    { name: 'Chocolate Belga', price: 0 }, { name: 'Quatro Leites', price: 0 },
    { name: 'Brigadeiro Branco', price: 0 }, { name: 'Beijinho', price: 0 },
    { name: 'Limão Siciliano', price: 0 }, { name: 'Mousse de Maracujá', price: 0 },
    { name: 'Ameixa', price: 0 }, { name: 'Pistache', price: 0 },
    { name: 'Doce de Leite', price: 0 }, { name: 'Abacaxi c/ Coco', price: 22 },
    { name: 'Mousse Flocado', price: 28 }, { name: 'Amêndoas', price: 32 },
    { name: 'Bombom', price: 32 }, { name: 'Geleia Frutas Amarelas', price: 38 },
    { name: 'Cereja', price: 38 }, { name: 'Nozes', price: 38 },
    { name: 'Damasco', price: 38 }, { name: 'Geleia Frutas Vermelhas', price: 42 },
    { name: 'Geleia Morango Fresco', price: 42 }, { name: 'Nutella', price: 42 },
  ];

  const coberturasOptions = [
    { name: 'Chantilly', price: 0 }, { name: 'Ganache', price: 0 },
    { name: 'Buttercream', price: 0 }, { name: 'Glacê', price: 0 },
  ...
  const docesComuns = [
    ...
    'Ninho com Amendoim', 'Casadinho de Ninho'
  ];

  const docesFinos = [
    'Morango Coberto', 'Coco com Ameixa', 'Coco com Damasco', 'Coco com Cereja'
  ];

  const docesGourmet = [
    'Brigadeiro Gourmet', 'Brigadeiro de Ninho', 'Brigadeiro de Limão',
    'Brigadeiro de Maracujá', 'Brigadeiro Capim Santo', 'Bombom de Uva',
    'Bombom de Banana', 'Bombom de Goiaba', 'Tortinha Cream Cheese',
    'Tortinha Limão', 'Tortinha Morango', 'Tortinha Maracujá c/ Coco',
    'Tortinha Physalis'
  ];
  ...
  const handleToggleOption = (item, currentList, setList, isPriceObj = true) => {
    ...
    if (isSelected) {
      setList(currentList.filter(i => (isPriceObj ? i.name : i) !== itemName));
    } else {
      if (maxSelections === 1 && !isDoce) {
        setList([item]); 
      } else {
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            ...
            {!isDoce && (
              <>
                ...
                <section>
                    <h3 className="text-lg font-bold text-[#4A3B32] mb-4">1. Escolha a Massa {maxSelections > 1 ? '(Até 2)' : '(Apenas 1)'} <span className="text-red-400 text-xs bg-red-50 px-2 py-1 rounded-full ml-2">* Obrigatório</span></h3>
                ...
                <section>
                    <h3 className="text-lg font-bold text-[#4A3B32] mb-4">2. Escolha o Recheio {maxSelections > 1 ? '(Até 2)' : '(Apenas 1)'} <span className="text-red-400 text-xs bg-red-50 px-2 py-1 rounded-full ml-2">* Obrigatório</span></h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <section>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    {docesOptions.map((doceName) => {
                        const isSelected = selectedDoces.includes(doceName);
                        return (
                            <button key={doceName} onClick={() => handleToggleOption(doceName, selectedDoces, setSelectedDoces, false)} className={`relative p-3 rounded-xl border-2 transition-all flex flex-col items-center justify-center min-h-[80px] ${isSelected ? 'border-[#D48D92] bg-[#D48D92] text-white shadow-lg scale-95' : 'border-gray-100 bg-white text-gray-700 hover:border-pink-200 hover:bg-pink-50'}`}>
                                {isSelected && <div className="absolute top-2 right-2"><CheckCircle size={16} /></div>}
                                <span className="font-bold text-center text-sm">{doceName}</span>
                            </button>
                        );
                    })}
                </div>
